    ...
    <style>
        ...
        * {
        ...
        .table td {
            padding: 10px;
            text-align: center;
            border-bottom: 1px solid var(--green8);
        }
                ...
                <div class="pengajuan">
                    ...
                    <div class="table">
                        <table>
                            ...
                            $sql = "SELECT pg.nama_pasien, p.id_pembayaran, p.status_pembayaran, 
                                           p.tanggal_pembayaran, u.tanggal_jadi,
                                           dp.biaya as jumlah_tagihan
                                    FROM pengujian pg
                                    JOIN pengujian u ON pg.id_pengujian = u.id_pengujian
                                    JOIN detail_pembayaran dp ON u.id_pengujian = dp.id_pengujian
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    $tanggal_pembayaran = $row['status_pembayaran'] === 'Belum Bayar' 
                                        ? '-' 
                                        : date('Y/m/d', strtotime($row['tanggal_pembayaran']));
                                    
                                    // Calculate Batas Pembayaran: 1 day after the end of the month of tanggal_jadi
                                    $date = new DateTime($row['tanggal_jadi']);
                                    $year = $date->format('Y');
                                    $month = $date->format('m');
                                    $batas_pembayaran = date('Y/m/d', strtotime("$year-$month-01 +1 month +1 day"));

                                    $status_class = $status_pembayaran === 'Sudah Bayar' ? 'verified' : 'pending';
                            ?>
                                    <tr>
                                        <td><?php echo $nama_pasien; ?></td>
                                        <td>Rp <?php echo $jumlah_tagihan; ?></td>
                                        <td><span class="status <?php echo $status_class; ?>"><?php echo $status_pembayaran; ?></span></td>
                                        <td><?php echo $batas_pembayaran; ?></td>
